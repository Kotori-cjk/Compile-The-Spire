...
`levels`中存放了18个主线关卡与11个EX关卡。

其中主线关卡为`grade_stats`,`text_palindorme`,`expression_eval`,`date`,`fraction`,`bank_account`,`complex_ops`,`vector_lite`,`inventory_lifecycle`,`shape_area`,`pet_shop`,`skill_system`,`task_scheduler`,`disjoint_set`,`maze_bfs`,`course_ranking`,`mini_interpreter`,`battle_simulator`来循序渐进地引导玩家填空完成完整程序。并在`answer`文件夹中附有源程序答案。

其中EX关卡引入`discard_drops`,`friendly_boss`,`bundle_pick`,`skip_on_full`,`no_discard`,`no_natural_discard`等多个新机制 special_tags ，引入资源规划/动态拓扑/背包控制等策略性，提升解谜难度与游戏趣味。
...
- `replaceCodeTemplateTokens`：是一个通用替换器：扫描space块或者clue块，然后调用传进来的 replacementForToken(tokenId)。根据调用方式决定具体替换成空格占位、线索文本、隐藏星号或代码块内容。
...
此外，项目还包含 `levels/answer` 中的参考答案源码。这些文件对课程验收和题目设计有参考价值，但当前游戏内判定主要依赖 JSON 中的匹配规则，而不是在运行时实际编译并执行玩家组合出的完整程序。

## 3. 小组成员分工情况

冯子涵（组长）：`LevelData`,`GameEngine`,`GameMap`,`Combat`,`ex-levels`,`SaveManager`。主要负责数据层的定义，游戏引擎、地图寻路、战斗和代码合成、存档等后端实现和EX1-11的关卡设计。

张然博：`Inventory`,`Levels`。主要负责背包状态、宝箱状态的后端实现，怪物图鉴的前端实现，1-18主线关卡设计，提供音效库，制作PPT与报告。

陈骏坤：`MainWindow`,`MapView`,`CodeBlockUI`。主要负责主窗口、关卡选择窗口、设置、交互控制、地图渲染、代码块、战斗部分的前端实现，生成美术素材，演示视频录制。
...
6. 关卡设计方面：通过special tag之间的组合可以进一步提升难度和玩法策略性。
